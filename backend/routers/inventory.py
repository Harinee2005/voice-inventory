...
def get_available_dates(db: Session = Depends(get_db)):
    ...
    return {"dates": [str(d[0]) for d in dates if d[0]]}
...
def get_storage_areas_list(db: Session = Depends(get_db)):
    ...
    return {"storage_areas": [a[0] for a in areas if a[0]]}
...
def get_all_inventory(
    ...
    return query.order_by(InventoryItem.category, InventoryItem.item_name).all()
